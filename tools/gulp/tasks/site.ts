...
task('serve:site', done => {
  detectPort(4200).then((port: number) => {
    execNodeTask('@angular/cli', 'ng', ['serve', 'doc', '--port', port === 4200 ? '4200' : '0'])(done);
  });
});

/** Run `ng build --prod --project=doc` */
task(
  'build:site-doc',
  execNodeTask('@angular/cli', 'ng', ['build', '--project=doc', '--prod', CI ? '--configuration=pre-production' : ''])
);

/** Run `ng build --prod --project=doc --configuration es5` */
task('build:site-doc-es5', execNodeTask('@angular/cli', 'ng', ['build', '--project=doc', '--prod', '--configuration=es5']));
